...
- [x] Cgroup resource limits — `cpu_limit=` (1–100, % of one core), `mem_limit=` (MB), and `cpuset=` (CPU affinity / core pinning, systemd `AllowedCPUs=` analog) per `.svc`; written via sync-pipe window before child exec; IEC 62304 Class C blast-radius isolation
