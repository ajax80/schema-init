...
static sdbus_replies *sdbus_replies_new(void) { return calloc(1, sizeof(sdbus_replies)); }

static void sdbus_replies_record(sdbus_replies *t, int callee, uint32_t serial, int caller) {
...
static int sdbus_replies_match(sdbus_replies *t, int from, uint32_t reply_serial) {
    for (int i = 0; i < t->n; i++)
...
static void sdbus_replies_purge(sdbus_replies *t, int conn) {
    for (int i = 0; i < t->n; i++)
        if (t->ents[i].live && (t->ents[i].caller == conn || t->ents[i].callee == conn))
            t->ents[i].live = 0;
}

static void sdbus_replies_free(sdbus_replies *t) {
    if (!t) return;
    free(t->ents); free(t);
}
